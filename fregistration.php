...
if (isset($_POST['submit'])) {
    ...
    if (empty($id) || empty($staff_no) || empty($name) || empty($phone) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        header("Location: financeregister.php");
        exit();
    }
    ...
    $stmt = mysqli_prepare($con, "SELECT * FROM finance WHERE email = ? OR staff_no = ?");
    ...
    $stmt = mysqli_prepare($con, "INSERT INTO finance(id, staff_no, name, phone, email, password) VALUES (?, ?, ?, ?, ?, ?)");
